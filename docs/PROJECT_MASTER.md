...
2. **U2_left ~1.0mm 한계** — offset sweep 정량 검증 완료 (`docs/U2_LEFT_OFFSET_SWEEP_2026-05-27.md`). corridor 유지 시 최선 개선 폭 ~0.046mm로 seam 재정의 정당화 부족. CAD 도면 또는 실물 측정 가능 시점까지 추가 분석 보류
3. **본사 실로봇 검증 — 현재 범위에서 제외** — Mech-Viz 라이브러리에 MZ07L 모델 실제 설치 확인 (2026-05-18). M3는 시나리오 B(로컬 시뮬 + 현장 실행 패키지)로 종결 — 5/26 3-seam phase3_first5 PASS 기준. Hand-Eye Calibration·실로봇 통신·실 데모 셀 정교화는 본사 협조 가능 시점까지 보류
...
7. **시뮬 셀 임시 설정** — 5/26 3-seam PASS는 작업물을 +300mm X/Y 이격한 임시 셀 기준. 실 데모 셀(작업물 위치, 지그, 테이블, TCP, 장애물 모델)로 옮기는 작업은 본사 협조 가능 시점까지 보류
8. **quality_gate_v2 corridor 임계 재검토 (M4 잔존)** — 안정성 리포트 (2026-05-28) 결과 `corridor_inlier_ratio` / `centerline_p90_mm` / `tangent_p90_deg`가 모든 캡처에서 동일값 (ref seam geometry 속성). 현 corridor_inlier_lt=0.90 임계는 U2_left를 항상 WARNING으로 만들어 verdict 무력화. 임계 자체 또는 측정 방식 재정의 필요 (`docs/STABILITY_REPORT_2026-05.md` §7 참고)

## 4. 전략 원칙

1. **Safety First** — Fail-dangerous 가능성이 있으면 기능 개발보다 우선 수정한다. 의도치 않은 더미/오염 데이터가 로봇에 들어갈 가능성을 원천 차단한다.
2. **MVP First** — 고급 기능(위빙, 고급 리샘플링, 실시간 트래킹)은 MVP 검증(시뮬 또는 실장비) 이후 단계적으로 착수한다.
...
| **Phase 2**: 시뮬 검증 + 시나리오 B 종결 | M3 (5월) | Mech-Viz 경유 MZ07L dry-run, 3-seam phase3_first5 PASS, 현장 패키지 | Mock 100회 파싱 실패 0, StopLatch 100%, MZ07L dry-run PASS (실로봇 통신은 본사 협조 가능 시점까지 보류) |
| **Phase 3**: 보유 데이터 기준 안정성 리포트 자동화 + 카메라 선정 | M4 (6월) | 보유 캡처 안정성 리포트 1차 완료 (2026-05-28), 카메라 A/B 최종 선정 | 안정성 리포트 산출, 카메라 선정 문서 |
...
| M3 (5월) | 시나리오 B 종결 — 본사 실로봇 검증은 현재 범위에서 제외 | Mech-Viz 경유 MZ07L dry-run, ROBOT_DELIVERY_MODE 토글, full transform 구조, 현장 패키지 | 5/26 3-seam phase3_first5 PASS ✅, 현장 패키지 30~60분 재현 (M5로 이관) |
| M4 (6월) | 보유 데이터 기준 안정성 리포트 자동화 | 보유 캡처 12+개 부위별 지표 변동 폭, 캡처 조건별 비교 표 | 단일 명령으로 안정성 리포트 1건 산출 ✅ (`docs/STABILITY_REPORT_2026-05.md`, 2026-05-28), corridor 임계 재검토 잔존 |
